...
mindmaps.ZoomController = function (eventBus, commandRegistry) {
    ...
    this.zoomTo = function (factor) {
        ...
        eventBus.publish(mindmaps.Event.ZOOM_CHANGED, factor, true);
    ...
    this.zoomIn = function (step) {
        ...
        if (this.zoomFactor > this.MAX_ZOOM) {
            ...
            eventBus.publish(mindmaps.Event.ZOOM_CHANGED, this.zoomFactor, false);
        }
        this.arrangeRedraw()
    ...
    this.arrangeRedraw = function () {
        window.clearTimeout(this.scaling)
        this.scaling = window.setTimeout(function () {
            self.zoomTo(self.zoomFactor)
        }, 300)
    }

    this.zoomToOne = function () {
        this.zoomFactor = 1
        eventBus.publish(mindmaps.Event.ZOOM_CHANGED, this.zoomFactor, true);
        return this.zoomFactor;
    }
