...
mindmaps.OpenDocumentView = function () {
    ...
    var $servertable = $dialog.find(".server-filelist");
    $servertable.delegate("a.title", "click",function () {
        if (self.serverDocumentClicked) {
            var t = $(this).tmplItem();
            self.serverDocumentClicked(t.data);
        }
    }).delegate("a.delete", "click", function () {
            if (self.deleteServerDocumentClicked) {
                var t = $(this).tmplItem();
                self.deleteServerDocumentClicked(t.data);
            }
        });
    /**
    ...
    this.renderServere = function (docs) {
        // empty list and insert list of documents
        var $list = $(".server-document-list", $dialog).empty();
        $("#template-open-table-item").tmpl(docs, {
            format: function (date) {
                if (!date) return "";
                var day = date.getDate();
     ...
     * @param {mindmaps.Document[]} docs
     */
    this.showOpenDialog = function (docs, serverDocs) {
        this.render(docs);
        this.renderServere(serverDocs);
        $dialog.dialog("open");
    };
...
mindmaps.OpenDocumentPresenter = function (eventBus, mindmapModel, view, filePicker) {
    ...
    view.serverDocumentClicked = function (doc) {
    ...
    view.deleteServerDocumentClicked = function (doc) {
        // TODO event
        mindmaps.ServerStorage.deleteDocument(doc, {
            success: function () {
                var docs = mindmaps.LocalDocumentStorage.getDocuments();
                docs.sort(mindmaps.Document.sortByModifiedDateDescending);
                mindmaps.ServerStorage.getDocuments({
                    success: function (serverDocs) {
                        serverDocs.sort(mindmaps.Document.sortByModifiedDateDescending)
                        view.showOpenDialog(docs, serverDocs);
                    },
                    start: function () {
                    },
                    error: function () {
