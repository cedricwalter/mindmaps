...
mindmaps.StatusBarPresenter = function (eventBus, view) {
    ...
    view.buttonClicked = function (id) {
        _(buttonIdPanelMap[id]).each(function (v) {
            v.toggle()
        });
    };
    ...
    this.addEntryN = function (panels, name) {
        var id = buttonCounter++;
        var $button = view.createButton(id, name);
        _(panels).each(function (p) {
            p.setHideTarget($button)
        });
        buttonIdPanelMap[id] = panels;
    };
};
